...
namespace Gif.Components
{
	public class AnimatedGifEncoder
	{
		...
		public bool Finish() 
		{
			...
			try 
			{
                Memory.WriteByte( 0x3b ); // gif trailer
                Memory.Flush();
				if (closeStream) 
				{
                    Memory.Close();
...
//			}
		...
		{
            Memory.WriteByte(0x21); // extension introducer
            Memory.WriteByte(0xf9); // GCE label
            Memory.WriteByte(4); // data block size
			int transp, disp;
			if (transparent == Color.Empty ) 
			{
				transp = 0;
            ...
            // packed fields
            Memory.WriteByte( Convert.ToByte( 0 | // 1:3 reserved
				disp | // 4:6 disposal
				0 | // 7   user input - 0 = none
				transp )); // 8   transparency flag

			WriteShort(delay); // delay x 1/100 sec
            Memory.WriteByte( Convert.ToByte( transIndex)); // transparent color index
            Memory.WriteByte(0); // block terminator
		}
		...
		{
            Memory.WriteByte(0x2c); // image separator
			...
			{
                // specify normal LCT
                Memory.WriteByte( Convert.ToByte( 0x80 | // 1 local color table  1=yes
					0 | // 2 interlace - 0=no
					0 | // 3 sorted - 0=no
					0 | // 4-5 reserved
		...
		protected void WriteLSD()  
		{
			...
			WriteShort(height);
            // packed fields
            Memory.WriteByte( Convert.ToByte (0x80 | // 1   : global color table flag = 1 (gct used)
				0x70 | // 2-4 : color resolution = 7
				0x00 | // 5   : gct sort flag = 0
				palSize) ); // 6-8 : gct size

            Memory.WriteByte(0); // background color index
            Memory.WriteByte(0); // pixel aspect ratio - assume 1:1
		}
		...
		{
            Memory.WriteByte(0x21); // extension introducer
            Memory.WriteByte(0xff); // app extension label
            Memory.WriteByte(11); // block size
			WriteString("NETSCAPE" + "2.0"); // app id + auth code
            Memory.WriteByte(3); // sub-block size
            Memory.WriteByte(1); // loop sub-block id
